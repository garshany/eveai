...
export async function createNativeResponse(input: {
  ...
  try {
    events = config.openai.responsesTransport === 'websocket'
      ? await requestWebSocketEvents(baseUrl, bodyPayload, timeoutMs, streamThisCall, input.signal)
      : await requestHttpSseEvents(baseUrl, bodyJson, timeoutMs, streamThisCall, input.signal);
  } finally {
    releaseResponseSlot();
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on('message', (raw: RawData) => {
      ...
      for (const frame of frames) {
        events.push(frame);
        maybeEmitTextDelta(frame, streamToActivity);
        if (
