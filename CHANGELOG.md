...
- **Live kill index** (`map_kill_events`): the global EVE-KILL feed poller that already ran for kill watches gained a second consumer that writes every killmail into one shared rolling table. This inverts the cost model — "what is happening in the four hundred systems around me" becomes a single indexed local query instead of dozens of outbound requests per viewer per refresh, and it is seconds fresh instead of the hour that ESI's aggregate endpoints offer. Bounded by age retention and a hard row cap, with a per-system-deduplicated cold-start backfill so a restart does not open onto a blank map.
- **Explainable danger scoring**: every score is returned as labelled terms — recency-weighted kill rate, repeat attackers, gate-camp signature, victim similarity to the pilot's own hull, the capability gap against the ship they are actually flying, the security floor, and a quiet discount. Without that last term every nullsec system paints red on security class alone and the map is useless exactly where it matters most. A red dot with no explanation is not intelligence, so the breakdown is not optional.
- **The agent speaks first.** The map's assistant panel is an ordinary `agent_threads` row marked `kind = 'perimeter'`, so history, the request queue, quotas, cancellation and SSE recovery all apply unchanged. Deterministic rules — pursuit by attacker identity, camp on the next hop, threat-level rise, value spike, capability gap, route degraded, security-band change, all-clear — write finished sentences with **no model call**; the model is asked for prose only for a danger-level situation and only once per its own longer cooldown, because a pilot flying for an hour must never cost a model call every five seconds. Repeats inside a cooldown collapse into one message with a counter, and every advisory carries an anchor so clicking it flies the camera to what it is about.
- **Risk-weighted routing**: Dijkstra over the local gate graph with cost `1 + λ·danger`, security preference as a penalty rather than a ban (a secure route to a nullsec station must still exist), avoid sets, and EVE-Scout wormhole edges. Every hop returns its own cost breakdown, and the route panel states how many of its systems danger data actually covered — the rest are scored zero, not "safe".
- **Four bounded agent tools**: `map_bubble_intel`, `route_risk`, `compare_ships`, `threat_explain`, each handing the model one prepared payload instead of a fan-out it would have to assemble and do arithmetic on.
- **Load discipline for a public deployment**: the five-second ESI position poll exists only while a stream is attached, runs once per character across every tab, sits under global and per-user caps that refuse with a retry hint rather than queueing, backs off exponentially, stops when the pilot logs off or the session idles, and drains on shutdown.
- **Honest about what it cannot know**, in the interface rather than in a footnote: EVE exposes no endpoint for who is in a system, so hostiles are always inferred from killmails and the map never claims otherwise; the ESI baseline is labelled hourly and never live; killmails show their age; position cannot be fresher than the five-second ESI cache; nothing reads the game client. Validation against this deployment's ESI catalog also found `/incursions/` and the faction-warfare endpoints absent, so those layers were cut from the design rather than promised.
- Rendering is a hand-written Canvas 2D layer plus five small d3 modules (`d3-zoom`, `d3-quadtree`, `d3-scale`, `d3-interpolate`, `d3-selection`). No graph library: the layout is deterministic, and a force simulation would destroy the jump-distance metric the whole feature rests on.
- Fixed the gate-camp radius, which had been wrong by a factor of a million: `GATE_CAMP_RADIUS_M` in `src/eve-board/analytics.ts` was `200_000_000_000` metres — 200 million km, about 1.3 AU, wider than most systems are across — while its own comment and docstring said 200 km. At that radius every ratting and mission kill in a system snapped to whichever gate happened to be nearest, so "kills clustered on one gate" carried no information. This fed the route briefings already and would have fed Perimeter's camp marker and its camp-on-next-hop advisory. Now 200 km, with `tests/unit/gate-attribution.test.ts` pinning the boundary (an observed camp kill sits ~15 km from the gate).
- Fixed a latent trap in `runMigrations` found while adding the schema: the legacy statement-by-statement fallback split `SCHEMA_SQL` on `;` without stripping `--` comments, so a semicolon inside a comment cut a `CREATE TABLE` in half and failed every legacy migration with "incomplete input". Comments are stripped before the split now.
