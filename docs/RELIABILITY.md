...
- SQLite remains canonical in both modes. Server-state drift or explicit provider state loss triggers an exact active-turn cold replay; transient transport failures retry the same continuation first — but only while the stream has produced no useful deltas yet (per the provider contract, an identical replay after useful content/tool deltas can duplicate a tool action, so the turn fails honestly instead), with jittered backoff so parallel turns do not retry in lockstep.
...
### Telegram delivery across restarts

- Pending updates are NOT dropped on boot by default (`TELEGRAM_DROP_PENDING_UPDATES=false`): messages sent while the bot was down (deploy/restart) are redelivered by Telegram and answered after startup.
- A staleness gate (`TELEGRAM_MAX_UPDATE_AGE_MINUTES`, default 15, 0 disables) skips redelivered updates older than the window, so a long outage does not replay a day-old backlog or stale destructive commands (`/clear`).
- Long polling confirms an offset only on the *next* `getUpdates`, so an update whose handler already ran is redelivered if the process dies in between — and with pending updates kept, it would be answered twice and its write tools re-run. Every update is therefore claimed in `telegram_processed_updates` before dispatch (`claimTelegramUpdate`), which makes handling at-most-once: a turn killed mid-flight is not retried (the user resends), while messages that arrived while the bot was **down** were never claimed and are still answered. The table is pruned by age (7 days, well past Telegram's redelivery window) on boot and every 500 claims.
- The gate applies only to updates **sent before this process started**: long polling handles updates one at a time, so a live message can legitimately wait behind a slow update for longer than the window and must never be skipped. Agent turns themselves run detached from the update handler (`runTelegramTurn` in `src/telegram/handlers.ts`), so one user's long turn does not stall polling for everyone else; the per-actor in-flight guard and the global active-request ceiling serialise turns instead.
- Button taps (callback queries) carry no press timestamp — `callbackQuery.message.date` is when the *bot* sent the keyboard — so they cannot be age-checked, and Telegram redelivers pending callbacks after a restart (replaying one silently switches the active character). A tap is therefore honoured only when its keyboard was sent by the running process: a redelivered tap necessarily belongs to a message from before the restart, so the comparison is exact and needs no clock window. Taps on older keyboards answer "отправь команду заново" instead of acting. Note the queue must not be probed to learn this — `getUpdates` with a negative offset confirms and discards everything before the update it returns, which would destroy the very backlog this section is about.
- Outgoing answers are formatted per message: chat-Markdown is converted to Telegram HTML (`markdownToTelegramHtml`), HTML-only text passes through, plain text is sent without a parse mode; on a Telegram entity-parse rejection the sender retries the original text as plain. Markdown decides first, because a mixed answer — a `plan_route` summary in Telegram HTML with the agent's Markdown around it — is a normal flow, and treating one tag as proof the whole message is formatted left `**bold**` on screen literally. The converter preserves Telegram's own tag set and its entities while escaping everything else, and a fenced block is recognised only at a line start so a same-line ` ```span``` ` cannot consume the text after it.
- Long answers are chunked by `splitForTelegram`, and each chunk is formatted independently. A split landing inside a ``` block closes the fence on that chunk and reopens it on the next, so no chunk can render trailing prose as code or swallow the text sharing a line with a stray fence.
...
- if the process is asked to stop while turns are running, it drains instead of exiting mid-answer. Ingress closes before anything else. Discord dispatch and web `enqueue()` close synchronously — Discord answers "перезапускается, повтори" rather than dropping the event, because the gateway never replays it to the next process, and web answers 503. Telegram is closed by `bot.stop()` instead of a flag: refusing an update in middleware still lets grammY confirm its offset, so the message would be lost rather than redelivered. That stop runs before every other await and is itself bounded — so a message arriving mid-shutdown cannot start a turn that shares the already-running deadline. The budget starts at the first line of shutdown, so every step shares it — stopping the EVE-KILL feed poller only cancels its sleep, and awaiting a poll that is inside a network call would otherwise consume the window before the drain began. In-flight turns then get up to `SHUTDOWN_DRAIN_MS` (default 600000 — matching the default `AGENT_TURN_DEADLINE_MS` so a default-length turn always fits, ceiling 3600000 matching the turn-deadline ceiling, 0 disables) to finish and reply, polled every `SHUTDOWN_DRAIN_POLL_MS`. The wait counts web-agent turns as well as Telegram/Discord ones: web requests run in their own coordinator and would otherwise be invisible to the drain and aborted by `server.close()`. Stopping the Telegram poller is itself bounded by the same deadline, since grammY waits on the current long poll and a stalled network would otherwise consume the entire window. The Discord gateway, the HTTP server, and the database close only after the drain, and `server.close()` is bounded by the same deadline — Fastify is built with `forceCloseConnections` so an open SSE stream cannot hold the stop past it. Turns still running at the deadline are logged by count before the exit — an abandoned answer is reported, never silent.
- known limitation of the drain: grammY fetches Telegram updates in batches, and `bot.stop()` only prevents the *next* poll. Updates already in the current batch keep being handled, so a stop arriving mid-batch can start turns inside the drain window and cut them at the deadline; because each is claimed in `telegram_processed_updates` before dispatch, those are not retried by the next process. They appear in the "N turn(s) still running" warning. Refusing them before the claim would let the next process re-fetch them (the batch's offset is never confirmed once polling stops), but the same refuse-in-middleware pattern silently loses updates whenever polling has *not* stopped, so the safer known-loss path is kept deliberately.
