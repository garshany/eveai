...
- if the process is asked to stop while turns are running, it drains instead of exiting mid-answer: producers and the Telegram poller stop accepting new work first, then in-flight turns get up to `SHUTDOWN_DRAIN_MS` (default 30000, ceiling 600000, 0 disables) to finish and reply, polled every `SHUTDOWN_DRAIN_POLL_MS`. The Discord gateway, the HTTP server, and the database close only after that window, so a turn that is still writing is not stranded. Turns still running at the deadline are logged by count before the exit — an abandoned answer is reported, never silent.
