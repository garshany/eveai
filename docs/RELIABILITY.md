...
- Outgoing answers are formatted per message: existing Telegram HTML passes through, the agent's chat-Markdown is converted to Telegram HTML (`markdownToTelegramHtml`), plain text is sent without a parse mode; on a Telegram entity-parse rejection the sender retries the original text as plain.
