...
Heartbeat — фоновая система уведомлений. Проверяет состояние EVE-персонажа по расписанию и отправляет сводку в Telegram, если есть изменения. Если ничего нового — молчит.
  ...
  └─ startHeartbeat(bot, db)
       └─ Cron('*/5 * * * *')          ← тикает каждые 5 минут
            └─ для каждого due юзера:
                 ...
                 ├─ saveState()           ← сохранить tracking state
                 ├─ runModelText()        ← модель суммирует findings
                 └─ bot.api.sendMessage() ← отправить в Telegram
```
...
Все findings за один тик собираются в один промпт и прогоняются через модель (`runModelText`). Модель формирует краткую сводку на русском языке для Telegram:
