...
- `OPENAI_MODEL=gpt-6-astra` is the default: the GPT-6 flagship. `gpt-6-sol`, `gpt-6-luna` (fast, cheap), `gpt-5.6-luna`, `gpt-5.6-terra` (capability/cost balance) and `gpt-5.6-sol` (quality-first) remain available. The `gpt-5.6` alias routes to Sol.
