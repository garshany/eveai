...
type ChatScreenProps = { title: string; messages: ChatMessage[]; busy: boolean; error: string | null; onMenu: () => void; onSend: (message: string) => Promise<void> };

export function ChatScreen({ title, messages, busy, error, onMenu, onSend }: ChatScreenProps) {
  const { t } = useI18n();
  ...
  return <section className="chat-canvas" style={{ '--chat-route-image': `url(${routeImage})` } as CSSProperties}>
    ...
    <div className="chat-scroll" aria-live="polite">
      ...
      {messages.length > 0 ? <div className="message-thread">{messages.map((message) => <MessageBubble key={message.id} message={message} />)}{busy ? <ThinkingMessage /> : null}<div ref={endRef} /></div> : busy ? <div className="message-thread"><ThinkingMessage /><div ref={endRef} /></div> : null}
    </div>
...
function ThinkingMessage() { const { t } = useI18n(); return <article className="message message--assistant message--thinking"><div className="assistant-mark"><CompassMark size={24} /></div><div className="thinking-dots" aria-label={t('thinking')}><span /><span /><span /></div></article>; }
